...
class GeneralUi:
    ...
    def _get_next_menu(self, name_of_current_menu: str, choosing_next_menu: int):
        ...
        current_dict_with_one_name_and_choices = self._menu_registry.get_controller().get(name_of_current_menu)
    ...
    def _validate_type_of_current_menu(self, menu_method_for_validating):
        ...
        if menu_method_for_validating in self._menu_registry.get_controller():
            return True

        return False

    def stream_program(self):
        """Метод, реализующий беск. цикл программы - безопасное переключение между методами"""

        current_menu = self._menu_registry.get_obj_of_start_menu()
...
class MenuRegistry:
    """Класс, хранящий словарь вида: {имя_меню: {выбор: следующий_метод}}"""

    def __init__(self, crud_menu_methods: CRUDmenuMethods,
                 menu_actions: MenuActions, exit_app: QuitApp):
        
        self._crud_menu_methods = crud_menu_methods
        ...
        self._controller = {
            "_menu_main" : {
                1: self._menu_actions._menu_view,
                2: self._menu_actions._menu_edit,
                3: self._exit_app._exit_application
            },
            "_menu_view" : {
                1: self._menu_actions._action_view_direct,
                2: self._menu_actions._action_view_reversed,
                3: self._menu_actions._menu_sort,
                4: self._menu_actions._menu_main
            },
            "_menu_edit" : {
                1: self._crud_menu_methods._menu_add,
                2: self._crud_menu_methods._menu_change,
                3: self._crud_menu_methods._menu_delete,
                4: self._menu_actions._menu_main
            }
        }
    ...
    def get_obj_of_start_menu(self):
        return self._menu_actions._menu_main
...
class MenuActions:
    ...
    def _action_view_direct(self):
        ...
        self.display_data_methods.display_transactions(data)
